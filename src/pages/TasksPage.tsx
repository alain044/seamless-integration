                ...
                <CardContent className="pt-0">
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <Badge variant="secondary" className={statusColor[task.status]}>
                      {task.status === 'completed' ? t('tasks.status.completedAwaitingApproval') : t(`tasks.status.${task.status}`, { defaultValue: task.status })}
                    </Badge>
                    ...
                    {task.status === 'approved' && task.approved_at && (
                      <span className="text-green-600 dark:text-green-400">✓ {t('tasks.status.approved')} {new Date(task.approved_at).toLocaleDateString()}</span>
                    )}
