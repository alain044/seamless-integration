...
const AuthPage = () => {
  ...
  const recordGoogleFailure = () => {
    ...
    try { localStorage.setItem(GOOGLE_FAIL_KEY, JSON.stringify(next)); } catch { /* storage unavailable (private mode/quota) */ }
    setGoogleAttempts(next.count);
    return next;
  };
  const clearGoogleFailures = () => {
    try { localStorage.removeItem(GOOGLE_FAIL_KEY); } catch { /* storage unavailable (private mode/quota) */ }
    setGoogleAttempts(0);
  };
