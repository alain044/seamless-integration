...
const Player = ({ b, onEvent }: {
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem('briefing_volume', String(volume)); } catch { /* storage unavailable — volume just won't persist */ }
  }, [volume, muted]);
...
const BriefingsPage = () => {
  ...
  // `load` is recreated on every render, so listing it as a dependency would
  // refetch in a loop. The user is the real trigger.
