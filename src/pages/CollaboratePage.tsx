...
const CasesTab = () => {
  ...
  useEffect(() => {
    ...
    // Keyed on the org id on purpose: the provider returns a new object on
    // every render, and depending on the object would refetch in a loop.
  ...
  useEffect(() => {
    ...
    return () => { supabase.removeChannel(ch); };
    // loadProfiles is recreated per render; see the cases effect above.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [active?.id]);
